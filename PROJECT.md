# VeriDAO Court — Project Rationale
...
On Ethereum, Kleros solved this: a Schelling-point-based decentralized court where random stake-weighted jurors vote honestly because coherence (voting with the majority) is the profitable strategy. It's live since 2019, 1000+ disputes settled.
...
**VeriDAO Court** is a general-purpose, Schelling-point-based decentralized arbitration court on Solana. Any Solana program can file disputes; the Court draws jurors, collects commit-reveal votes, and emits rulings — all governed by game-theoretic incentives, not trusted humans.

### Core mechanism

```
1. A program files a Dispute (via CPI): subcourt, options, evidence hash, fee
2. The Court randomly draws N Jurors from the Subcourt (Switchboard VRF, weighted by stake)
...
- **Subcourts.** Specialized juror pools (automotive, dental, freelancing, NFTs). Jurors self-select by expertise. Permissionless creation — anyone can register a Subcourt.
- **Per-Subcourt staking token.** Each Subcourt defines which SPL token Jurors stake (USDC by default; any token). The stake is the anti-sybil mechanism and the coherence-slashing substrate.
- **Arbitrable interface.** Any Solana program can use the Court: `create_dispute()` → `get_ruling()`. Two CPI calls. The Court has no knowledge of the filing program's domain.
...
The Court doesn't depend on mutuals or any specific application. It's a general-purpose primitive — the "Kleros of Solana." Any program that needs subjective dispute resolution can use it:
...
The Court ships first, proves the Schelling mechanism on Solana, then the Mutual layer plugs in on top.
...
- Not a mutual product (the mutual is a client of the Court, built separately)
- Not Kleros (it's a new Solana-native implementation with Switchboard VRF, per-Subcourt tokens, and the Arbitrable CPI interface)

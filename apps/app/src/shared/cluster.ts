/**
 * cluster.ts — Solana cluster list + RPC endpoint env-var reading.
 *
 * Feeds ConnectorKit's `getDefaultConfig({ clusters })` (accord-y5av).
 * Vite exposes `VITE_*` env vars to the client; RPC URLs are rate-limited
 * private endpoints, not secrets (see apps/app/.env.example).
 */

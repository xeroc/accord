 ...
 *
 * {@link describeError} pulls a clean reason out of a
 * {@link TransactionSendError}'s program logs (Anchor "Error Code: X. Error
 * Message: Y." → "X: Y") and falls back to {@link unwrapError} for everything
 * else (wallet rejections, SDK errors). Mirrors apps/app.
 */
...
// Error Message: bar." — that pair is the nicest to show, so it wins.
...
/**
 * Turn any error into a short, user-facing message.
 *
 * - `TransactionSendError` (pre-flight simulation revert): extracts the program
 *   error from its logs. Falls back to the RPC error code when the simulation

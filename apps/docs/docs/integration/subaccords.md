...
| `jurors_per_dispute` | `u32`      | Round-0 panel size.                                                                                             |
...
- Mutable only via 48h timelock (`propose_subaccord_update` → `execute_subaccord_update`): `min_stake`, `jurors_per_dispute`, `alpha_bps`, `review/commit/reveal_window`, `max_appeals`, `fee_per_juror`, `authority`, `evidence_operator`.
...
accord::create_subaccord(
    ...
    jurors_per_dispute, alpha_bps,
    review_window, commit_window, reveal_window,
    max_appeals, fee_per_juror,
...
```typescript
import { createSubaccord, findSubaccordPda } from "@useaccord/sdk";

...
- *Case for allowing 0:* low-stakes pools may want a ruling the instant a round resolves — no appeal right, capital freed immediately. `finalize_dispute` becomes eligible at `reveal_end` (since `reveal_end + 0 = reveal_end`).
- *Case against 0:* the appeal is the Schelling safety valve — a wrong round-1 ruling gets overturned by a larger panel. A 0-window pool has no recourse against a captured/thin round-1 panel. Also a footgun: a creator who forgets to set it gets instant-finality-by-default, which is the *opposite* of the safe default.

---
# accord-8m2a
title: Subaccord dispute-kit config — aggregation enum + rename jurors_per_dispute → initial_num_jurors (ADR-0014)
status: todo
type: feature
priority: high
...
ADR-0014 (Accepted) locks two `Subaccord`-account changes that make the per-Subaccord mechanism config ("dispute kit") explicit rather than hardcoded:
...
- [ ] Docs already updated: ADR-0014, SPEC.md Subaccord row + out-of-scope, EVIDENCE-FORMAT.md §4 + §10 cross-ref, ADR index
...
ADR-0014 · `programs/accord/SPEC.md` (Account model, Out of scope) · `state.rs` Subaccord · `apps/evidence-daemon/EVIDENCE-FORMAT.md` §4/§10 · constants (`MAX_JURORS`)

...
- Docs: SPEC.md instruction row 1 now matches the real signature (also fixed pre-existing risk_type/rules_hash drift); packages/canon/README.md createList facade documented (was claiming not-yet-shipped). Anchor.toml [programs] canon entry auto-synced by anchor to declare_id (was stale).

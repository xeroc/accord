...
export async function execute(
  ...
  if (d.data.state !== DisputeState.Final && d.data.state !== DisputeState.Failed) {
    return {
  ...
  const appealBondAddr = await appealBondPda(ctx.programId, d.address, action.roundIdx);
  const bond = await fetchAppealBond(ctx.accord.rpc, appealBondAddr);
  if (bond.data.amount === 0n) return { skipped: "bond already claimed/zeroed" };
  ...
  const ix = claimAppealRefund(ctx.accord.adapter, ctx.programId, accounts, action.roundIdx);
...
export function register(d: CrankDispatch): void {
  registerCrank(d, "claim_refund", execute);
}

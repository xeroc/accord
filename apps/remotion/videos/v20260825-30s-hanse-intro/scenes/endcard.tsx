import { Interactive, useCurrentFrame, useVideoConfig } from "remotion";

import { enterAt } from "../../../src/shell/anim";
import { AccordMark, AmberRule, Wordmark } from "@useaccord/ui";
import { Scene } from "../../../src/shell/scene";

/** S6 · END CARD — wordmark, tagline, honest status, one link. */
export function EndcardScene() {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  return (
    <Scene seed="hanse-endcard" stack className="gap-7">
      <Interactive.Div name="Endcard mark" style={{ opacity: enterAt(frame, fps, 0.05, 0.35) }}>
        <AccordMark
          size={90}
          progress={enterAt(frame, fps, 0.05, 0.35)}
          className="text-amber"
        />
      </Interactive.Div>
      <Interactive.Div name="Endcard wordmark">
        <Wordmark
          enter={enterAt(frame, fps, 0.1, 0.4)}
          settle={24}
          brandName="Hanse"
          className="text-8xl"
        />
      </Interactive.Div>
      <AmberRule enter={enterAt(frame, fps, 0.4, 0.4)} />
      <Interactive.Div
        name="Endcard tagline"
        className="font-mono text-3xl text-text-secondary"
        style={{ opacity: enterAt(frame, fps, 0.6, 0.4) }}
      >
        the protocol for mutuals.
      </Interactive.Div>
      <Interactive.Div
        name="Endcard status"
        className="font-mono text-2xl text-text-secondary"
        style={{ opacity: enterAt(frame, fps, 1.1, 0.5) }}
      >
        contributions live on mainnet · disputes live on devnet · the mutual in build
      </Interactive.Div>
      <Interactive.Div
        name="Endcard link"
        className="font-mono text-3xl text-amber"
        style={{ opacity: enterAt(frame, fps, 1.5, 0.4) }}
      >
        hanse.useaccord.xyz
      </Interactive.Div>
    </Scene>
  );
}

...
| GET    | `/evidence/{subaccord}/{dispute}[/{round}]` | → `200` raw stored `EvidenceBundle` (ciphertext only). Public — no auth, no re-encryption. `404` if no bundle stored. `round` defaults to `0`.                                                                                                                                                               |

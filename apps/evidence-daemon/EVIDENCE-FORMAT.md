...
description: "The auth module shipped on 2026-07-28 per spec.\nSee timeline.md for the commit log." # optional markdown claim body (§3.2); JSON-escaped, single line
...
| `description`                     | no       | Optional markdown claim body (the challenger's argument). Serialized as one JSON-escaped line (`description: "<escaped>"`); markdown newlines are emitted as `\n`. **Omitted entirely when absent/empty** → byte-identical to a pre-`description` manifest (sha256 stable). Rendered display-only — committed bytes are never altered; Jurors still verify `sha256(manifest) == evidence_hash` over the raw bytes. |

...
| `entries[].path`                  | yes      | URL or relative POSIX path, UTF-8. No `..`, no backslash. Unique within the package. URLs accepted for MVP (real leaf `sha256` transport is v2 — daemon path A).                                                                                                 |
| `entries[].sha256`                | yes      | Lowercase hex (64 chars) of the file's bytes. All-zero sentinel (`0000…0000`) ⇒ juror skips leaf verification for this entry (root gate `sha256(manifest) == evidence_hash` still applies). v2 will enforce real leaf hashes once per-file blob transport lands. |

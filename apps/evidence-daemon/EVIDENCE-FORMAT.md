...
| `entries[].path`                  | yes      | URL or relative POSIX path, UTF-8. In MVP, entries are URLs (e.g. `https://example.com/doc.md`). Relative POSIX paths (no leading `/`, no `..`, no backslash) are also accepted for future blob transport (v2). Unique within the package. |
| `entries[].sha256`                | yes      | Lowercase hex (64 chars) of the file's bytes. All-zero sentinel (`"0000…0000"`) ⇒ Juror skips leaf verification (root gate via `evidence_hash` still applies). MVP default for URL entries where the filer hasn't computed the leaf hash.  |
| `policy`                          | no       | Operator hints (non-binding on the mechanism): `watermark`, `retain_until`, etc.                                                                                                                                                           |

> **MVP entry note:** v1 ships URL-path entries with the all-zero sha256
> sentinel — the filer provides a URL to each evidence file, and Jurors
> verify only the root gate (`sha256(manifest) == evidence_hash`). Real leaf
> sha256 values (enabling per-file Merkle verification) and multi-MIME blob
> transport (daemon path A) are deferred to v2.

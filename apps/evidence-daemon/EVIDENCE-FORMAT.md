...
| `entries[].path`                  | yes      | A URL (any scheme the operator/Juror can fetch) **or** a relative POSIX path, UTF-8. Relative paths: no leading `/`, no `..`, no backslash, unique within the package. v1 implementation entries are URL-or-sentinel; the relative-path archive-bundle model is v2 (§7). |

/**
 * Domain CAS routes (ADR-0027). Pure wiring: validate the hex route param,
 * read the raw body, delegate to injected handlers, map results to HTTP.
 * No domain logic lives here — hash/size/idempotency checks are
 * {@link ../pipeline/domain.ts}.
 *
 *   PUT /domains/:hash   → domain put handler (201 created / 200 no-op)
 *   GET  /domains/:hash  → domain get handler (bytes + stored Content-Type,
 *                          ETag = hash, Cache-Control: immutable)
 *
...
export function domainRoutes(deps: ServerDeps): Hono {
  ...
  app.put("/domains/:hash", async (c) => {
    ...
    const res = await deps.domainPut(hash, bytes, contentType);
    if (!res.ok) {

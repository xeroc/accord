...
| --------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Subaccord`     | `["subaccord", creator, risk_type_hash]` | `staking_token` (collateral mint, ADR-0002/0020), `fee_token` (compensation mint, ADR-0020), `min_stake`, `alpha_bps` (slash = `alpha_bps * min_stake / 10_000`), `review/commit/reveal_window`, `max_appeals` (the sole per-Subaccord panel-shape knob; round-1 size is the fixed `INITIAL_NUM_JURORS` = 3, ADR-0019), `aggregation: Aggregation` (v1 = `Plurality`; the dispute-kit tally rule, ADR-0019), `fee_per_juror` (in `fee_token`), `authority: Pubkey` (`default` ⇒ immutable), `evidence_operator: Pubkey`. `risk_type` + `evidence_spec` immutable. **Accumulator:** `root_hash: [u8;32]`, `total_stake: u64`, `next_index: u32`, `depth: u8` (fixed at creation, default 20 ≈ 1M seats). |
...
| `Dispute`       | `["dispute", filer, nonce]`              | `subaccord`, `options: [..; MAX]`, `evidence_hash: [u8;32]`, `state`, `current_round`, `final_ruling: u8`, `fee_paid: u64` (running available fee pool, ADR-0020), `committed_vrf: Option<[u8;32]>`, `frozen_root: [u8;32]` (set in `commit_vrf_callback`), `frozen_total_stake: u64`                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 9   | `appeal(dispute)`                                         | **permissionless**; pays `N_new · fee_per_juror` + bond (in `fee_token`); opens a new round at `2N+1`. Max 3 appeals. Draws from the same `frozen_root`.                                                                                                                                                                                                                                                         |
| 10  | `finalize_round` / `finalize_dispute`                     | permissionless crank. `finalize_round`: tallies, credits `fees_earned += fee_per_juror` per revealer, decrements `fee_paid`. `finalize_dispute`: redistributes slash pool (`stake_delta`, stake_token) + fee pool (`fees_earned`, fee_token) to coherent jurors; decrements `active_draws`.                                                                                                                      |
...
- **Fee credit:** `finalize_round` credits each revealer `fees_earned += fee_per_juror` and decrements `dispute.fee_paid`. (Reveal-quorum threshold gate is ADR-0021.)
...
Accord token · dynamic params · non-Plurality aggregation (IRV / median — ADR-0019) · stake-weighted Subaccord self-governance (ADR-0005 alt) · Arcium encrypted vote-tally (Juror vote privacy) · reveal-quorum threshold + shortfall redraw (ADR-0021, E2) · any on-chain evidence crypto beyond the trusted-operator hash model · validity proof (SNARK) that the accumulator root was built correctly (ADR-0012 future).

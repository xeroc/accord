...
export function stake(
  ...
  const ix = client.buildStake({ programId, accounts, amount, path });
  // RECLAIM-LEAF: append the freed JurorStake PDA as remaining_accounts[0]
  // when staking into a recycled slot.
  if (freedSlotAccount) {
    return {
      ...ix,
      accounts: [

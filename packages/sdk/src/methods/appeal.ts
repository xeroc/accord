...
/** Max drawn jurors per round (constants.rs: MAX_JURORS). */
export const MAX_JURORS = 31;

/** Appeal window after `reveal_end` (constants.rs: APPEAL_WINDOW_SECS). */
export const APPEAL_WINDOW_SECS = 3 * 24 * 60 * 60;
...
export function panelSizeForRound(
  ...
  const factor = 1 << roundIdx; // 2^roundIdx (roundIdx < 31 ⇒ fits)

...
/** 48h update timelock in slots (400ms/slot). propose→execute minimum wait. */
export const UPDATE_TIMELOCK_SLOTS = 432_000n;

/** 24h unpause timelock in slots — a freeze is always recoverable on schedule. */
export const UNPAUSE_TIMELOCK_SLOTS = 21_600n;

/** Program max on `Subaccord.max_appeals` (constants.rs: MAX_APPEALS). */
 ...
 * `evidence_spec` are immutable and absent. Mirrors the on-chain Borsh enum;
 * the generated codec encodes the tag, the facade passes this union through.
 */
export type UpdatePayload =
  | { kind: "MinStake"; value: bigint }
  | { kind: "JurorsPerDispute"; value: number }
  | { kind: "AlphaBps"; value: number }
  | { kind: "ReviewWindow"; value: bigint }
  | { kind: "CommitWindow"; value: bigint }
  | { kind: "RevealWindow"; value: bigint }
  | { kind: "MaxAppeals"; value: number }
  | { kind: "FeePerJuror"; value: bigint }
  | { kind: "Authority"; value: Address }
  | { kind: "EvidenceOperator"; value: Address };
...
export async function createSubaccord(
  ...
  creator: Address,
  args: CreateSubaccordArgs,
): Promise<{ instruction: IInstruction; subaccord: Address; bump: number }> {
...
export async function proposeSubaccordUpdate(
  ...
  nonce: bigint,
  payload: UpdatePayload,
): Promise<{ instruction: IInstruction; pendingUpdate: Address }> {
  if (nonce < 0n || nonce > U64_MAX)
    throw new Error(`InvalidNonce: expected u64, got ${nonce}`);
...
export async function initializePause(
  ...
  programId: Address,
  authority: Address,
): Promise<{ instruction: IInstruction; pauseState: Address }> {

 ...
 * -owned ATA of the Subaccord `fee_token` (the single escrow mint, ADR-0020)
 * — lazily created on first `join` (canon precedent).
 *
 * @see ADR-0010
 */

import { findAssociatedTokenAddress, findDisputePda } from "@useaccord/sdk";
...
// Milestone-lane alias (accord-nsxa facade): the generated name, namespaced.
 ...
 * (ADR-0010). */
export async function findCaseVaultPda(
  feeMint: Address,
  casePda: Address,
): Promise<Address> {

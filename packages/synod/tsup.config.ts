...
/**
 * Bundle the Synod SDK into ESM.
 *
 * Same rationale as `packages/canon/tsup.config.ts`: `tsc` emits relative
 ...
 * output therefore breaks `@useaccord/synod` under Node — e.g. the PDA smoke
 * test (`node --test`) once `pda.ts` sources `SYNOD_PROGRAM_ADDRESS` from the

...
- **Two harnesses, complementary** (decision veridao-8ys4):
  - **LiteSVM + host unit** (`programs/<p>/tests/*_litesvm.rs`, `src/tests.rs`, `make test_unit`) — fast in-process Rust per instruction. Wired via `anchor-litesvm` from the xeroc fork (branch `litesvm-0.16-sbpfv3`, brimigs/anchor-litesvm#3 / PR #4) carrying the litesvm 0.16 (agave 4.2) bump — upstream 0.4.0 pins litesvm ^0.11/agave 3.1.x, whose sbpf cannot load the canonical sBPFv3 ELF. Swap to the registry release as soon as upstream ships one (bean accord-jhzr). The safe-solana-builder `references/litesvm.md` is the **checklist/pattern authority** for every instruction bean (happy/auth/reinit/time-lock/arithmetic/closure, sysvar, CU) — wiring differs, concepts don't.
...
- **Toolchain note (sBPFv3):** programs build as **sBPFv3** bytecode (SIMD-0178/0189/0377 — static syscalls, no runtime relocations). Anchor 1.2.0 defaults to `--arch v3 --tools-version v1.57`; the Makefile pins the same via `ANCHOR_BUILD_FLAGS`. `cargo build-sbf` invoked directly MUST pass both flags too — agave bundles older platform-tools as its default (3.1.10 ships v1.52), and `--arch v3` on tools older than v1.53 silently emits non-v3 bytecode. Verify with `make verify-sbf` (`readelf -h target/deploy/<p>.so` → `Flags: 0x3`). SIMD-0500 will reject deployments/upgrades of v0–v2 bytecode; already-deployed programs keep executing.
- **Host unit tests** (`programs/<p>/src/tests.rs`) pin cross-cutting invariants (scoped VRF identity, MST accumulator math; the manual layout-offset pins were removed with the offset consts — `remaining_accounts` are fully Anchor-deserialized, ADR-0032) — they run under `make test_unit` too.
